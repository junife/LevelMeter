...
if(Dx < 0) // jesli sk³adowa pozioma jest ujemna 
  {
  Xinc = -1; // to bêdziemy si?"cofa? (krok ujemny)
...
if (Dy < 0) // jeœli sk³adowa pionowa jest ujemna
  {
  Yinc = -1; // to bêdziemy si?"cofa? (krok ujemny)
...
if ((Dx != 0) || (Dy != 0)) // sprawdzamy czy linia sk³ada si?z wiêcej ni?jednego punktu ;)
  {
  ...
  if (Dy <= Dx) // jeœli tak, to idziemy "po iksach"
    ...
    do // ruszamy w drog?
      ...
      if(TwoDxAccumulatedError > Dx)  // jeœli TwoDxAccumulatedError jest wiêkszy od Dx
        {
        CurrentY += Yinc; // zwiêkszamy aktualn?pozycj?w pionie
